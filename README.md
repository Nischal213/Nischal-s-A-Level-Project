...
def login_system():
    ...
    def next():
      ...
      if question_number == 2:
        ...
        if button['state'] == 'normal':
          ...
          elif len([i for i in username
                    if i in string.digits]) == len(username):
            change = True
  ...
  def login_page():
    ...
    def checking():
      ...
      if not (pass_flag) and error_string:
        if attempt == 6:
          attempt = 0
          error_message[
            'text'] = 'Too many failed attempts in a row, Temporarily on lockdown for 30 seconds'
    ...
    password_entry.pack()
    check_button = Button(text='Enter',
                          command=checking,
                          font=('Microsoft JhengHei UI', 13),
                          width=10)
    ...
    error_message.pack()
    forgot_button = Button(text='Forgot Password',
                           command=forgot,
                           font=('Microsoft JhengHei UI', 13),
                           width=20)
...
def easy_game_UI():
  ...
  label.pack()
  question_box = Message(text=f'What is {num1} {rand_op} {num2}?',
  ...
  button = Button(text='Enter',
                  ...
                  command=checker)
  button.pack()
  point_box = Message(text=f'Points: {points}',
                      font=('Comic Sans MS', 14),
                      width=350)
  point_box.place(x=250, y=230)
  lives_box = Message(text=f'Lives: {lives}',
                      font=('Comic Sans MS', 14),
                      width=350)
...
def medium_game_UI():
  ...
  question_box = Message(text=f'{question_msg}',
                         font=('Comic Sans MS', 17),
                         width=350)
  question_box.pack(pady=20)
  answer_box = Entry()
...
def game_over_screen():
  ...
  retry = Button(text='Play again',
                 font=('Microsoft JhengHei UI', 13),
                 width=15,
                 command=retry_button)
  retry.pack(pady=10)
  lb = Button(text='See Leaderboards',
              font=('Microsoft JhengHei UI', 13),
              width=15,
              command=lb_button)
  lb.pack(pady=10)
...
def display_lb():
  ...
  def shift_lb():
    ...
    if position == 0:
      ...
      first_place[
        'text'] = f'🏆🥇 {medium_lb[0][0]} scored {medium_lb[0][1]} in {medium_lb[0][2]} difficulty'
      second_place[
        'text'] = f'🏆🥈 {medium_lb[1][0]} scored {medium_lb[1][1]} in {medium_lb[1][2]} difficulty'
      third_place[
        'text'] = f'🏆🥉 {medium_lb[2][0]} scored {medium_lb[2][1]} in {medium_lb[2][2]} difficulty'
    elif position == 2:
      first_place[
        'text'] = f'🏆🥇 {easy_lb[0][0]} scored {easy_lb[0][1]} in {easy_lb[0][2]} difficulty'
      second_place[
        'text'] = f'🏆🥈 {easy_lb[1][0]} scored {easy_lb[1][1]} in {easy_lb[1][2]} difficulty'
      third_place[
        'text'] = f'🏆🥉 {easy_lb[2][0]} scored {easy_lb[2][1]} in {easy_lb[2][2]} difficulty'
